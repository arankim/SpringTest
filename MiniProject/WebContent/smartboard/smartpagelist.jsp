...
<%
	...
	//페이징 처리에 필요한 변수들	
	int totalCount=db.getTotalSmartCount(); //총 글의 갯수
	...
	Vector<SmartDto> list=db.getSmartDatas(startNum,endNum);
...
%>
 ...
 onclick="location.href='layoutgaro.jsp?main=smartboard/smartboard.jsp'">  
 ...
 <div style="margin-left: 5px;">
 	...
 	{%>
		<tr>
			...
			<td>
				<a style="color: black;" 
				href="layoutgaro.jsp?main=smartboard/content.jsp?num=<%=dto.getNum()%>&pageNum=<%=currentPage%>&key=list">
				<%=dto.getSubject()%></a>
			</td>	
 ...
 %>
	...
	<%
	 ...
	 {%>
		<li>
		<a href="layoutgaro.jsp?main=smartboard/smartlist.jsp?pageNum=<%=startPage-1%>">
		◀</a></li> 
	 <%}
	 %>	  
	 ...
	 {%>
		...
		href="layoutgaro.jsp?main=smartboard/smartlist.jsp?pageNum=<%=i%>"><%=i%></a>
		</li> 
	 <%}
	 %>	
	 ...
	 {%>
		<li>
		<a href="layoutgaro.jsp?main=smartboard/smartlist.jsp?pageNum=<%=endPage+1%>">
		▶</a></li> 
	 <%}
	 %>	 
	 </ul>	
