	...
	*/
	...
	public static void main(String[] args)
	{
	    ...
	    for(int k=0; k<30; k++)
	    {
		    ...
		    // Encryption		
		   SHA256_Encrpyt( pbData, Plaintext_length, pbCipher ); // SHA256_Init, SHA256_Process, SHA256_Close 함수를 내부적으로 모두 호출하여 
		                                                                     //  간편한 인터페이스를 제공하기 위한 함수 
	...
	// Encryption			    
	    ...
	    byte chageByte[]  = testData.getBytes();
	    byte pszDigest[] =  new byte[32];
	 
	    System.out.println(chageByte.toString());
	    System.out.println("chageByte length :" + chageByte.length);
	    System.out.println("test data : " + testData);
	    KISA_SHA256.SHA256_Encrpyt(chageByte, chageByte.length, pszDigest);
	   
	    for (int i=0; i<32; i++) System.out.print(Integer.toHexString(0xff&pszDigest[i]));
